...
class SpookyPage extends MusicBeatState
{
    var bg:FlxSprite;
    var buyNow:FlxSprite;
    var owned:FlxSprite;

    var spookyPage:FlxSprite;
	...
	var spookyHatsAmount:Int = 200;

	var alreadyBought:FlxText;

	public static var boughtSPOOKYHats:Bool = false;
	...
	override public function create()
	{
		...
		spookyPage = new FlxSprite(50, 170).loadGraphic(Paths.image('channelsin/wiishop/characters/buyingState/chars/SPOOKY'));
		add(spookyPage);
	...
	{
		add(blackScreen);
		bought = new FlxText("Just bought Spooky Kids With Wario And Luigi Hats!");
		bought.setFormat(Paths.font("vcr.ttf"), 40, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		bought.screenCenter();
		add(bought);
	...
	override function update(elapsed:Float)
	{
		...
		if (FlxG.mouse.overlaps(buyNow))
		{
			...
			{
				if (WiiShopChannel.wiiPoints >= spookyHatsAmount)
				{
					if (FlxG.save.data.boughtSPOOKYHats == false)
					{
						Bought();
						boughtSPOOKYHats = true;
						SelectYourPlayableChar.displaySpooky = true;
						WiiShopChannel.wiiPoints -= spookyHatsAmount;
						save();
						FlxG.save.data.displaySpooky = true;
					}
					else if (FlxG.save.data.boughtSPOOKYHats == true)
					{
		...
		FlxG.save.data.boughtSPOOKYHats = boughtSPOOKYHats;
    }

	public static function checkIfBought():Bool
	{
		if (FlxG.save.data.boughtSPOOKYHats != null)
			return FlxG.save.data.boughtSPOOKYHats;

		return boughtSPOOKYHats;
	}
